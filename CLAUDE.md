...
- `max_utility` — **Phase-4 routing override** (not a per-agent behavior). When checked, the per-agent shuffle-and-decide loop is skipped and `_assign_agents_centrally()` runs instead: deciding agents are sorted by their best-possible utility and processed in descending order so high-value agent-ride pairs lock in first. Each assignment updates `pending_arrivals` live, so later agents see realistic queue pressure. Hunger triggers and decision paralysis are bypassed in this routing path (intentional simplification for the social-planner benchmark). Useful as a "what's the best a centralized planner could do" reference against the decentralized Nash equilibrium baseline.
