...
%----------------------------------------------------------------------
\section{Appendix: Figure and Data Index}
%----------------------------------------------------------------------

All simulations were run with the simulation engine in \texttt{simulation.py}, the network model in \texttt{park\_model.py}, and the on-demand backend server in \texttt{server.py}. Data files and figures are organized as follows:
...
\toprule
\textbf{Analysis} & \textbf{Generating Script} & \textbf{Data File} \\
\midrule
Strategy comparison (rational) & \texttt{generate\_report\_data.py} & \texttt{results.csv} \\
Strategy comparison (behaviors) & \texttt{generate\_report\_data\_behaviors.py} & \texttt{behaviors\_results.csv} \\
Express adoption sweep (rational) & \texttt{express\_sweep.py} & \texttt{express\_sweep\_results.csv} \\
Express adoption sweep (behaviors) & \texttt{behaviors\_express\_sweep.py} & \texttt{behaviors\_express\_sweep\_results.csv} \\
Preselect adoption sweep (rational) & \texttt{preselect\_paid\_sweep.py} & \texttt{preselect\_paid\_sweep\_results.csv} \\
Preselect adoption sweep (behaviors) & \texttt{behaviors\_preselect\_paid\_sweep.py} & \texttt{behaviors\_preselect\_paid\_sweep\_results.csv} \\
\bottomrule
\end{tabular}
\caption{Reproducibility index. Each script consumes the simulation engine and emits both a CSV of summary statistics and PNG figures into the \texttt{figures/} directory.}
\end{table}

All figures referenced in this report are stored in the \texttt{figures/} directory of the project repository. Per-strategy histograms, queue time-series, and population time-series are named \texttt{\{strategy\}\_\{histogram,queues,population\}.png} (rational baseline) or \texttt{behaviors\_\{strategy\}\_\{histogram,queues,population\}.png} (with behaviors). Cross-strategy comparison charts are named \texttt{comparison\_\{happiness,avg\_metrics,distributions\}.png} or \texttt{behaviors\_comparison\_\{...\}.png}. Composite sweep figures are at \texttt{express\_sweep.png}, \texttt{behaviors\_express\_sweep.png}, \texttt{preselect\_paid\_sweep.png}, and \texttt{behaviors\_preselect\_paid\_sweep.png}.
