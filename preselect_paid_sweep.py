"""
Paid Preselect adoption sweep.

Runs Paid Preselect at adoption levels 0%, 10%, ..., 100% and produces a single
composite figure (mirrors express_sweep.py exactly):
  - Holder utility distribution heatmap (rows = adoption %)
  - Non-holder utility distribution heatmap (rows = adoption %)
  - Line plot of holder/non-holder/system mean utility + gap ratio vs %

Output: figures/preselect_paid_sweep.png and preselect_paid_sweep_results.csv.
"""
...
def main():
    print(f"Running {len(ADOPTION_LEVELS)} Paid Preselect sweep simulations (seed={SEED})...\n")
    ...
    csv_path = Path(__file__).parent / "preselect_paid_sweep_results.csv"
    ...
    fig.suptitle("Paid Preselect Adoption Sweep — Utility Distribution & Diminishing Returns",
                 fontsize=14, y=0.98)

    out = FIG_DIR / "preselect_paid_sweep.png"
