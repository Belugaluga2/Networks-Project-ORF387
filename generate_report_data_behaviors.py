"""
Generate strategy-comparison data and figures WITH ALL AGENT BEHAVIORS ENABLED.

Mirrors generate_report_data.py but every sim has all 5 imperfect behaviors active
...
Per-run stats → behaviors_results.csv. Per-strategy figures and cross-strategy
comparisons are saved with the `behaviors_` prefix:
  - figures/behaviors_{strategy}_histogram.png
  - figures/behaviors_{strategy}_queues.png
  - figures/behaviors_{strategy}_population.png
  - figures/behaviors_comparison_happiness.png
  - figures/behaviors_comparison_avg_metrics.png
  - figures/behaviors_comparison_distributions.png
"""
...
def run_strategy(strategy: str, seed: int):
    """Run a single full-day sim with all 5 behaviors enabled.
    Returns (summary, history, agent_records)."""
...
def plot_histogram(strategy: str, agents: list[dict]) -> None:
    ...
    ax.set_title(f"{PASS_LABELS[strategy]} (All Behaviors) — Utility Distribution")
    ax.grid(axis="y", alpha=0.3)
    fig.tight_layout()
    fig.savefig(FIG_DIR / f"behaviors_{strategy}_histogram.png", dpi=140)
    plt.close(fig)
...
def plot_queues(strategy: str, history: list[dict]) -> None:
    ...
    ax.set_title(f"{PASS_LABELS[strategy]} (All Behaviors) — Park-Wide Queue Length")
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(FIG_DIR / f"behaviors_{strategy}_queues.png", dpi=140)
    plt.close(fig)
...
def plot_population(strategy: str, history: list[dict]) -> None:
    ...
    ax.set_title(f"{PASS_LABELS[strategy]} (All Behaviors) — Park Population")
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(FIG_DIR / f"behaviors_{strategy}_population.png", dpi=140)
    plt.close(fig)
...
def plot_comparison_happiness(rows: list[dict]) -> None:
    ...
    ax.set_title("Total Park Happiness by Strategy — All Behaviors Enabled (mean ± std over 3 seeds)")
    ax.tick_params(axis="x", labelrotation=15)
    ax.grid(axis="y", alpha=0.3)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "behaviors_comparison_happiness.png", dpi=140)
    plt.close(fig)
...
def plot_comparison_avg_metrics(rows: list[dict]) -> None:
    ...
    for ax, (key, title) in zip(axes, metrics):
        ...
        ax.grid(axis="y", alpha=0.3)
    fig.suptitle("Per-Strategy Aggregates — All Behaviors Enabled (mean ± std over 3 seeds)", y=1.02, fontsize=13)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "behaviors_comparison_avg_metrics.png", dpi=140)
    plt.close(fig)
...
def plot_comparison_distributions(per_strategy_agents: dict) -> None:
    ...
    ax.set_xlabel("Total Happiness")
    ax.set_ylabel("Number of Park-goers")
    ax.set_title("Utility Distributions Across Strategies — All Behaviors Enabled")
    ax.legend(loc="upper right", fontsize=9)
    ax.grid(axis="y", alpha=0.3)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "behaviors_comparison_distributions.png", dpi=140)
    plt.close(fig)
...
def main():
    ...
    print(f"Running {len(PASS_STRATEGIES) * len(SEEDS)} simulations with all 5 agent behaviors enabled...\n")
    for strategy in PASS_STRATEGIES:
        for seed in SEEDS:
    ...
    csv_path = Path(__file__).parent / "behaviors_results.csv"
    ...
    print("=" * 90)
    print(f"\nAll output in {FIG_DIR.relative_to(Path.cwd())}/ and behaviors_results.csv")
